# train_kpconv_bjss_attention_jl.py
import os
import json
import torch
...
from data_loader_kpconv import TomatoDataset
from model_kpconv_attention_jl_4unet import KPConvTomato
import numpy as np
from tqdm import tqdm
from datetime import datetime

# 基于kpconv的训练脚本（茎叶Iou、训练结果迭代、边界感知损失函数、ReduceLROnPlateau）
...
def main():
    ...
    # 替换原 Cosine scheduler
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode='max',  # monitor val mIoU
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch + 1}: "
              ...
              f"LR={optimizer.param_groups[0]['lr']:.6f}")  # 实时显示当前 lr
        ...
        # ✅ 关键：传入 val_mIoU 给 ReduceLROnPlateau
        scheduler.step(epoch_mean_iou)  # ←←← 放这里！
    ...
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=0, pin_memory=False,
                             collate_fn=KPConvTomato.collate_fn)
