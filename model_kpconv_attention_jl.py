...
class BoundaryAwareAttention(nn.Module):
    def __init__(self, k=16, radius_factor=1.2, stem_enhance=True, leaf_edge_protect=True):
        ...
        # 动态参数占位符 (实际值在forward中动态计算)
        self.stem_radius = None
        self.stem_axis_weight = None
        self.leaf_edge_density_threshold = None
        self.leaf_edge_weight = None

    # ========== 动态计算茎半径 ==========
    def _estimate_stem_radius(self, stem_pts):
        """动态估计茎的物理半径 (单位: 米)"""
        if stem_pts.shape[0] < 10:
            return 0.007  # 默认7mm
        
        # 计算茎点云局部密度 (使用6近邻)
        dists = torch.cdist(stem_pts, stem_pts)
        dists.fill_diagonal_(1e6)  # 排除自身
        ...
        estimated_radius = 0.02 / (mean_density + 1e-6)  # 基准值2cm
        
        # 限制在合理范围 [3mm, 15mm]
        return torch.clamp(estimated_radius, min=0.003, max=0.015).item()
    ...
    def _compute_stem_prior(self, points, labels):
        ...
        # 动态计算茎半径
        self.stem_radius = self._estimate_stem_radius(stem_pts)
        self.stem_axis_weight = min(0.8, 0.4 + num_stem / 100.0)  # 点数越多权重越高 [0.4, 0.8]
        ...
        # 使用动态估计的半径
        norm_dists = perp_dists / self.stem_radius
    ...
    # ========== 动态计算叶边缘参数 ==========
    def _estimate_leaf_edge_params(self, leaf_pts):
        """动态估计叶边缘保护参数"""
        if leaf_pts.shape[0] < 30:
            return 0.3, 2.0  # 默认值
        ...
        # 动态阈值: 取密度分布的25%分位数
        self.leaf_edge_density_threshold = torch.quantile(densities, 0.25).item()
        
        # 动态权重: 与最小密度成反比
        min_density = densities.min()
        max_density = densities.max()
        density_range = max_density - min_density + 1e-6
        
        # 基础权重2.0 + 密度差异增强
        self.leaf_edge_weight = 2.0 + 1.5 * (1.0 - min_density / (max_density + 1e-6))
        self.leaf_edge_weight = max(1.5, min(3.5, self.leaf_edge_weight))  # 限制在[1.5, 3.5]
        
        return self.leaf_edge_density_threshold, self.leaf_edge_weight
    ...
    def _compute_leaf_edge_prior(self, points, labels):
        ...
        # 动态估计叶边缘参数
        density_thresh, edge_weight = self._estimate_leaf_edge_params(leaf_pts)
